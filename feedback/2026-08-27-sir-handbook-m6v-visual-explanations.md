...
lane: sdd
toolVersion: 1.4.0
commit: 7f21882a26d0e5fb6dd9667139875ae4e3e56dee
---
...
The qualification deliberately separates structural cost from observed readiness. It records exactly six diagrams, 138 SVG elements, 16,561 bytes, and eight animated elements against the typed-spec limits; it then measures 30 warm same-browser no-store navigations from navigation start until all six SVG images load, decode, and have non-zero intrinsic dimensions. The resulting p95 53.6 ms and p99 56.1 ms pass the unchanged 100/200 ms candidate limits. Browser launch (125.142 ms), cold-site readiness (141.2 ms), and caller/template tail are observations outside that gating subject. Six source/accessibility/fallback/structural mutations and one permanent in-subject timing mutation each observed red and restored green. Strict docs, 24 diagram-by-mode checks, nine screenshots, the full client glyph/Battlefield qualifier, and the existing Quint/runtime correspondence suite passed.
...
#### §4.2 One typed declaration remained stable through the measurement repair
...
- **Observed:** Commit `6204e70` and final commit `7f21882` both carry workload `handbook-m6v-six-diagram-render-v1`, the same definition digest, six-diagram scope, structural limits, 100/200 ms limits, and `headless-browser` capability. The repaired exact-candidate p95/p99 are 53.6/56.1 ms. Structural output is 138 elements, 16,561 bytes, and eight animated elements. No separate earlier commit proves when the numeric limits were first chosen.
- **Evidence:** commit:6204e70c823ac9706568067d4d951b344de6cbb6; commit:7f21882a26d0e5fb6dd9667139875ae4e3e56dee; file:work/377-handbook-m6v/spec.md; file:work/377-handbook-m6v/plan.md; file:readiness/377-handbook-m6v/performance-smoke.json; file:readiness/377-handbook-m6v/performance-evidence.json; file:readiness/377-handbook-m6v/visual-qualification.json
...
- **Observed:** The repaired route uses six external `<img>` resources and a browser-init observer plus `HTMLImageElement.decode()` with non-zero intrinsic dimensions. It reports warm p95/p99 53.6/56.1 ms separately from 125.142 ms browser launch and 141.2 ms cold-site readiness. A retained 350 ms in-subject response-delay mutation records p95/p99 399.5/409.9 ms against the unchanged 100/200 ms limits; the full owner command then restores green. Twenty-four diagram-by-mode observations and nine screenshots passed after clipping and print-contrast repairs.
...
- **Version:** S.I.R. commit `7f21882`; Playwright 1.62.1; Chromium 151.0.7922.34; FsDocs 22.1.0.
...
No authority contradiction remains. The concrete visual binds exact production glyph and palette tokens plus the representative authoritative rule path. Each abstract visual binds exact Quint declaration/rule identities and source digests. The manifest expressly denies semantic authority, and the audit makes any bound authoritative-source change invalidate the diagrams. The first human render did reveal a presentation contradiction: SVG source bounds passed structurally while the right-edge caption clipped in the actual page and print contrast was weak. Rendered inspection, not prose, drove the repair.
...
One route refusal required manual structured-route authoring. Performance intent needed one canonical-key re-baseline. Three rendered timing runs were discarded while isolating the correct clock and replacing nested contexts; a screenshot pass required clipping and print-contrast repairs. Evidence required one canonical ordering/sample-artifact fixpoint. These costs did not change the fixed performance budgets or semantic scope. Full aggregate qualification was rerun after qualification-script changes; focused strict docs and render checks were rerun after final roadmap synchronization.
...
- Visual structure: six diagrams, 138 elements, 16,561 bytes, eight animated elements; six source/accessibility/fallback/structural red-restored mutations plus one retained timing red-restored control.
- Rendered route: strict FsDocs handbook, 24 diagram-by-mode observations, nine screenshot receipts, zero console/page errors.
- Performance: 30 warm samples after three warmups; p95 53.6 ms, p99 56.1 ms against unchanged first-candidate 100/200 ms limits; launch and cold-site observations separate; retained 350 ms in-subject mutation p95/p99 399.5/409.9 red before restored green.
...
- Preserve §4.2 by keeping workload `handbook-m6v-six-diagram-render-v1`, its digest, six-diagram scale, structural limits, 100/200 ms thresholds, and headless/no-compositor capability consistent from spec through standard evidence. Acceptance: final spec, plan, smoke, manifest, and evidence contain one matching declaration; do not infer a pre-implementation commit that does not exist.
- Preserve §4.3 by running the browser-init decoded-image probe on the actual strict FsDocs route. Acceptance: p95/p99 derive only from the 30 warm in-browser readiness samples; browser launch and cold-site remain separately labeled; a permanent in-subject delay over 200 ms fails without changing budgets.
...
- Preserve fallbacks and human rendering. Acceptance: normal, reduced-motion, print, and effects-off each retain identical labeled mechanics for all six diagrams; non-WebGL/static document text remains complete; screenshots show no clipping or contrast loss.
...
| testing | exercised | Source binding, accessibility, fallbacks, semantic fingerprints, 24 mode checks, screenshots, seven red/restored mutations, strict docs, glyph/Battlefield, and Q4/runtime passed. |
...
| performance | exercised | Typed workload/digest/scale/budgets, PERF-SMOKE, structural counters, 30 warm decoded-image samples, cold separation, and a retained in-subject overflow mutation passed; independent pre-implementation numeric ownership is not claimed. |
