...
lane: sdd
toolVersion: 1.0.1
commit: a9a66de937dcc668508e16c4b1f77ed712fdd1dd
---

## §1 Provenance and confidence

This report describes the third repaired source boundary at commit `a9a66de937dcc668508e16c4b1f77ed712fdd1dd`, based on `origin/main` commit `ee6a2df`. The cycle used .NET SDK 10.0.302, Fable 5.13.0, Node v26.7.0, npm 12.0.2, and SDD CLI 1.0.1. Six checkpoint events are recorded. One source-frozen local aggregate reached the old paired wall-time assertion and failed there; it exited before writing a durable timing receipt, so this report does not treat its exact duration or complete preceding subject inventory as audited evidence. No aggregate retry was run. The first hosted run exposed six transport, binding, aggregation, timing, and protected-route defects. Its first replacement exposed early output inventory, unrelated evidence prerequisites, and a 315-second same-runner producer path. The second replacement verified those repairs but exposed a 324 MB global prepared superset, documentation closure omissions, mutable-intermediate sealing, a missing browser runtime, and producer timing collapsed out of runner-minutes. Focused mutation proofs cover the narrow per-producer correction, while replacement exact-head hosted CI, review confirmation, merge, and post-merge obligations remain pending.
...
The versioned route, candidate/route/artifact-bound gate receipts, all-failure deterministic join, mode-preserving immutable transport, and explicit workflow DAG make the PR acceptance surface inspectable and independently attributable. Route-selected native, Fable, web, server, and documentation producers run on separate runners and publish narrow content-addressed outputs; each gate waits for and downloads only its declared producers, so there is no global preparation join on the critical path. Mutable `obj` files are excluded, documentation owns its locked Release API closure, and browser installs the selected .NET runtime. Lifecycle metadata is neutral when mixed with a product change, so adding SDD or feedback records does not promote a focused product route to cross-cutting; metadata-only changes retain an evidence-only route. Focused route, join, workflow, scheduled-full mutation, archive, late-output, mode, mutable-intermediate, phase-failure, clean-checkout evidence, and receipt tests found defects quickly without an aggregate loop.

## §3 What did not

The one aggregate reached item #215's frozen-baseline assertion after the qualification surface had changed. The collector observed a negative comparison, but the failed run did not preserve a tracked timing receipt or complete baseline-versus-candidate inventory comparison. The correction kept the threshold unchanged on an explicit paired experiment and made normal local/protected commands emit clean-room timing without comparing an expanding surface to a permanent baseline. The first hosted run then showed mode, evidence-cache, binding, aggregation, timing, and protected-route defects. Round one repaired those findings, but its replacement showed early output discovery, unrelated evidence prerequisites, incomplete failure receipts, and a 315-second same-runner path. Round two moved producers to separate runners and repaired those contracts. Its replacement showed that a global 324 MB superset still delayed every consumer, mutable `obj` trees invalidated otherwise passing gates, docs lacked its locked package/API closure, browser lacked .NET 10, and verdict runner-minutes hid individual producer work. Round three replaced the global artifact with narrow producer receipts and direct gate dependencies, excluded mutable intermediates, added docs/browser prerequisites, and made producers first-class timed verdict subjects. Replacement hosted evidence remains pending.
...
No scaffold creation, gameplay behavior change, package publishing, dependency upgrade, or interactive runtime playtest was exercised. The round-three replacement hosted PR DAG, externally unobservable queue timing, merge, and post-merge done stamp remain pending.
...
The stale paired comparison consumed the only local aggregate. Diagnosis and all three review repairs used static/focused checks and did not restart the aggregate. The failed local command did not preserve a tracked terminal timing receipt, so exact elapsed cost is intentionally omitted. The round-one hosted replacement consumed 315 seconds before the multi-runner correction; the round-two replacement exposed the costly global transport and prerequisite defects without a retry. SDD evidence required one correction from authored pass declarations to an observed JUnit receipt; no product source changed during that correction.
...
Route, direct producer dependency, join, workflow, scheduled/protected-edge mutation, archive/late-output/mode/mutable-intermediate receipt, Fable inventory, clean-checkout evidence, failed-phase timing, shell-syntax, and diff focused checks passed at the third repair boundary. The sole local aggregate reached the historical timing assertion and failed there; its complete preceding pass inventory is not claimed as durable evidence, and no retry occurred. SDD has 49/49 observed evidence declarations, verification readiness, and ship readiness. Round-three exact-head hosted CI and confirmation by the same independent reviewer remain pending.
