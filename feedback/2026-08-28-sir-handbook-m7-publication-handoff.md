...
modes, 48 screenshots total, with p95 64.9 ms and p99 68.1 ms under the unchanged M6V 100/200 ms
load/decode budgets. Eight isolated M7 mutations each observed red before untouched green.
After an exact-Node replay exposed host-sensitive tail variance, two quiescent 100-sample comparison
batches passed at p95/p99 61.0/66.5 ms and 63.0/64.6 ms. The M7 validity wrapper observes and enforces
...
The first exact-head implementation critique then found that chapter 47 still advertised the immutable
M6V self-test after the successor handbook changed and that the owner qualifier accepted any Node 26
while recording the declared 26.5.0 identity. The repair points chapter 47 at the M7 successor audit and
requires observed `node --version` to equal `v26.5.0`; the final qualification uses that exact runtime.
A later exact-Node reviewer replay observed p95/p99 148.8/239.8 ms while its temporary per-sample output
was removed on exit. A following controlled attempt observed 228.5/264.3 ms while unrelated concurrent
.NET/MSBuild work consumed the shared host, whereas an immediately preceding identical run passed at
73.3/82.4 ms. The new wrapper therefore records host validity and compares two quiescent batches. Its
first implementation falsely counted its own Chromium descendants as competitors despite a green
73.5/85.9 ms batch; PPid-based ownership repaired that classification before acceptance evidence was sealed.
...
- **Observed:** The verify/ship checkpoint reports three retired candidates and four replays; discarded outputs are not retained as independent receipts. Current evidence independently verifies that four final approvals bind handbook `a8f6b9ac…`, model `2347769…`, and manifest `7a84db6…`; fresh render digest `8bd0a685…` records 48 captures and p95/p99 64.9/68.1 ms. The validity receipt adds observed exact-Node quiescent comparison batches at 61.0/66.5 ms and 63.0/64.6 ms with zero invalid samples or competitors.
- **Evidence:** file:work/380-handbook-m7/publication-reviews.json; file:work/380-handbook-m7/publication-record.json; file:readiness/380-handbook-m7/rendered/inspection.json; file:readiness/380-handbook-m7/render-measurement-validity.json; command:node work/380-handbook-m7/audit-publication-handoff.mjs --self-test
...
- Sources: handbook `a8f6b9ac…`; model preserved `2347769…`; maintenance `151395f…`; eight source blobs and seven tools checked.
...
- Render: six diagrams, five modes, 48 fresh screenshots, retained replay p95/p99 64.9/68.1 ms, plus observed exact-Node quiescent comparison batches at 61.0/66.5 ms and 63.0/64.6 ms under unchanged 100/200 ms budgets.
- Publication: eight M7 publication and five host-validity red/restored controls; strict FsDocs and successor build selector green under the pinned host environment.
- Delivery: SDD evidence/verify/ship and the first exact-head critique completed; final exact-head acceptance, hosted PR CI, merge, exact-main/Pages/live proof, issue Done, and cleanup remained pending at this report boundary.
...
| testing | exercised | Four subject reviews, M6 structure/link gate, Q4 regression, 48 render captures, eight publication mutations, and five host-validity mutations passed. |
| evidence | exercised | Exact review/render/identity, two-batch host-validity, and SDD verify/ship receipts exist at the report boundary. |
| runtime-playtest | not-exercised | Runtime correspondence is retained regression evidence, not a new gameplay journey. |
| performance | exercised | Existing M6V workload replayed at p95/p99 64.9/68.1 ms and compared in two quiescent observed exact-Node batches at 61.0/66.5 and 63.0/64.6 ms without a new performance intent or budget. |
...
| worker-git-pr | partial | Isolated claim, implementation commits, and first exact-head critique exist; final acceptance, PR/CI/merge/post-merge proof remained pending. |
