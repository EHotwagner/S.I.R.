...
The first AST implementation over-constrained navigational links and treated aliases/inline code as ordinary controlled prose. After correction, exact-head review found the opposite failure: symbol prose was skipped, any destination was accepted for a controlled link, generated definitions could be tautological or truncated, and declaration/rule/alias removals lowered observed counts instead of failing. Replacement reviews then found controlled terms could hide inside longer link labels and a coordinated manifest/index term removal could lower 188 to 187. The repair now exempts only explicit structural/per-term cases, checks exact and embedded-label canonical destinations under a manifest-declared ambiguity policy, requires substantive definitions, independently asserts the exact 188-term cardinality, and reconciles exact model declarations, Quint/runtime rule IDs, and published alias markers against fixed cardinalities. Eligible prose and authority removal remain fail-closed.
