...
The aggregate runs `mutate-stale-reuse`, which changes a real bound output, requires the production verifier's `output-identity-drift` refusal, and restores the original bytes in an unconditional cleanup boundary. The driver also records baseline/candidate wall milliseconds, Fable target build counts, retained subject inventory, and reduction basis points under `artifacts/qualification/single-pass-timing.json`. Timing is acceptance evidence, not a runtime correctness threshold.
