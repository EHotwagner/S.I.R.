...
Pull-request qualification does not publish a repository-wide build tree. Independent native, Fable, web, server, and documentation producers each seal one content-addressed archive, production receipt, artifact manifest, phase timing, and typed result. Their inventories contain only immutable outputs required by consumers; `obj` directories and unrelated `bin` trees are never transported. The native producer builds `SIR.slnx` once in Release and packages only the required runtime closures, replacing the former Debug solution plus serialized Release-owner sequence. Rules consumes native; spatial and cross-runtime consume native plus Fable; cancellation consumes native plus web; browser consumes web plus server; documentation consumes web plus its own locked Release API closure; evidence consumes no prepared artifact. Every consumer verifies transport, candidate, route, input/tool/command, file content, and permission modes before use, with no fallback build.
