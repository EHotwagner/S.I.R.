...
- Pull requests run the focused lane in `.github/workflows/ci.yml`: changed paths are classified, the always-on integrity floor runs, and route-selected native, Fable, web, server, and documentation producers publish separate narrow content-addressed artifacts. Independent named gates then run concurrently; the cancellation mutant reuses the verified native test fixture instead of rebuilding it, while compiling the mutated web subject in isolation. `pr-verdict` joins every typed result. Every omitted gate has a route-policy reason; unknown or mixed product paths select the conservative cross-cutting route.
