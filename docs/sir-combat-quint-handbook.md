...
<figure class="combat-quint-diagram" data-diagram-embed="attack-pipeline"><object type="image/svg+xml" data="assets/sir-combat-quint-interactive/attack-pipeline.svg" aria-label="Production rifleman and contact split into a blocking-cover stop branch and a separate unblocked armor-and-damage branch" style="width:100%;aspect-ratio:720/300;display:block"><a href="assets/sir-combat-quint-interactive/attack-pipeline.svg"><img src="assets/sir-combat-quint-interactive/attack-pipeline.svg" alt="Production rifleman and contact split into a blocking-cover stop branch and a separate unblocked armor-and-damage branch" /></a></object><figcaption>Hover the unit, trace, branches, armor, or consequence for detailed mechanics. The established battlefield glyph and overlay vocabulary separates a cover-stopped impact from the representative unblocked attack outcome. Motion and glow are optional emphasis. <a href="assets/sir-combat-quint-interactive/attack-pipeline.svg#effects-off">effects-off view</a></figcaption></figure>
<details id="diagram-transcript-attack-pipeline"><summary>Attack pipeline diagram transcript</summary><p>A production rifleman occupies its faction-outlined unit footprint. Its authoritative trace reaches first contact, where two distinct outcomes separate. On the upper branch, projectile-blocking cover consumes the collision, stops the projectile, and produces zero target harm. On the lower, unblocked branch, armor retains eighty percent; the resulting twenty-point harm leads to an injury classification, stress, and collateral fields in one atomic consequence. The named rule bindings are COMBAT&#45;TRACE&#45;002, COMBAT&#45;COLLISION&#45;001, COMBAT&#45;COVER&#45;003, COMBAT&#45;ARMOR&#45;004, COMBAT&#45;DAMAGE&#45;001, COMBAT&#45;HEALTH&#45;001, COMBAT&#45;WOUND&#45;001, COMBAT&#45;SUPPRESSION&#45;001, COMBAT&#45;COLLATERAL&#45;001, and COMBAT&#45;ATTACK&#45;RESOLUTION&#45;001. Static geometry, labels, caption, and this transcript carry the complete meaning.</p></details>
...
<figure class="combat-quint-diagram" data-diagram-embed="rule-dependency"><object type="image/svg+xml" data="assets/sir-combat-quint-interactive/rule-dependency.svg" aria-label="Eight direct rule dependencies converge on the aggregate attack-resolution rule" style="width:100%;aspect-ratio:720/360;display:block"><a href="assets/sir-combat-quint-interactive/rule-dependency.svg"><img src="assets/sir-combat-quint-interactive/rule-dependency.svg" alt="Eight direct rule dependencies converge on the aggregate attack-resolution rule" /></a></object><figcaption>Hover any dependency, arrow, aggregate node, or scope note for details. The aggregate rule's eight authoritative direct dependencies converge on the final composite result; explanatory sequencing remains a separate deterministic sequence. <a href="assets/sir-combat-quint-interactive/rule-dependency.svg#effects-off">effects-off view</a></figcaption></figure>
<details id="diagram-transcript-rule-dependency"><summary>Rule dependency diagram transcript</summary><p>Engagement, collision, cover, perforation, harm, injury, stress, and collateral each have one arrow directly into COMBAT&#45;ATTACK&#45;RESOLUTION&#45;001. This is the aggregate rule's immediate dependency collection from the catalogue, not the wider transitive graph. The arrows do not claim execution order or hidden intermediate states.</p></details>
...
<figure class="combat-quint-diagram" data-diagram-embed="state-action"><object type="image/svg+xml" data="assets/sir-combat-quint-interactive/state-action.svg" aria-label="Checked atomic transition from a cohesive pre-state and attack input to a post-state and observation" style="width:100%;aspect-ratio:720/240;display:block"><a href="assets/sir-combat-quint-interactive/state-action.svg"><img src="assets/sir-combat-quint-interactive/state-action.svg" alt="Checked atomic transition from a cohesive pre-state and attack input to a post-state and observation" /></a></object><figcaption>Hover either state, the input, enabling check, operation, observation, or transition arrows for details. A checked transition maps one cohesive pre-state and input to one post-state and observation; invalid input disables the operation. <a href="assets/sir-combat-quint-interactive/state-action.svg#effects-off">effects-off view</a></figcaption></figure>
<details id="diagram-transcript-state-action"><summary>State and transition diagram transcript</summary><p>The authoritative initial pre-state has vitality one hundred, stress zero, barrier integrity one hundred, active blocking, and remains capable. Attack input enters a validity predicate. The atomic consequence produces a complete post-state with vitality eighty, stress twelve, unchanged barrier fields, and continued capability, plus a separate observation with harm twenty and no injury classification at that threshold. Invalid input disables the operation, so it creates neither a transition nor a hidden partial state.</p></details>
...
<figure class="combat-quint-diagram" data-diagram-embed="q4-arithmetic"><object type="image/svg+xml" data="assets/sir-combat-quint-interactive/q4-arithmetic.svg" aria-label="Twenty-five times full trace retention times eighty-percent armor retention equals twenty" style="width:100%;aspect-ratio:720/230;display:block"><a href="assets/sir-combat-quint-interactive/q4-arithmetic.svg"><img src="assets/sir-combat-quint-interactive/q4-arithmetic.svg" alt="Twenty-five times full trace retention times eighty-percent armor retention equals twenty" /></a></object><figcaption>Hover each operand, operator, result, flow, or determinism note for its exact calculation. Raw scale-ten-thousand operands make the representative twenty-point calculation visible without floating point. <a href="assets/sir-combat-quint-interactive/q4-arithmetic.svg#effects-off">effects-off view</a></figcaption></figure>
...
Hover a meaningful cell, ray, blocker, gate, counter, or result node for its detailed native SVG tooltip. Each figure
is embedded as a same-origin interactive SVG object so those element-level tooltips remain available in the handbook;
its nested image fallback remains visible in repository Markdown, the caption link opens the standalone diagram, and
the transcript carries the same essential meaning without hover.
...
<figure class="combat-quint-diagram" data-diagram-embed="supercover-walk"><object type="image/svg+xml" data="assets/sir-visibility-quint/supercover-walk.svg" aria-label="Four numbered orthogonal grid cells connect a blue observer to a yellow target using the canonical x-first supercover" style="width:100%;aspect-ratio:960/430;display:block"><a href="assets/sir-visibility-quint/supercover-walk.svg"><img src="assets/sir-visibility-quint/supercover-walk.svg" alt="Four numbered orthogonal grid cells connect a blue observer to a yellow target using the canonical x-first supercover" /></a></object><figcaption>Hover grid squares, paths, glyphs, and corner markers for details. The direct ray is explanatory; the numbered orthogonal walk is the executable blocker path. Animated marching and glow are optional emphasis. <a href="assets/sir-visibility-quint/supercover-walk.svg#effects-off">effects-off view</a></figcaption></figure>
...
<figure class="combat-quint-diagram" data-diagram-embed="canonical-symmetry"><object type="image/svg+xml" data="assets/sir-visibility-quint/canonical-symmetry.svg" aria-label="Opposite A-to-B and B-to-A requests pass through one canonical endpoint ordering and produce the same cell sequence" style="width:100%;aspect-ratio:960/430;display:block"><a href="assets/sir-visibility-quint/canonical-symmetry.svg"><img src="assets/sir-visibility-quint/canonical-symmetry.svg" alt="Opposite A-to-B and B-to-A requests pass through one canonical endpoint ordering and produce the same cell sequence" /></a></object><figcaption>Hover requests, flows, the canonical gate, or the result sequence for details. Both caller directions become the same ordered question before traversal. <a href="assets/sir-visibility-quint/canonical-symmetry.svg#effects-off">effects-off view</a></figcaption></figure>
...
<figure class="combat-quint-diagram" data-diagram-embed="two-traces"><object type="image/svg+xml" data="assets/sir-visibility-quint/two-traces.svg" aria-label="Parallel lanes compare a four-cell supercover terrain trace with a three-cell sampled boundary trace" style="width:100%;aspect-ratio:960/500;display:block"><a href="assets/sir-visibility-quint/two-traces.svg"><img src="assets/sir-visibility-quint/two-traces.svg" alt="Parallel lanes compare a four-cell supercover terrain trace with a three-cell sampled boundary trace" /></a></object><figcaption>Hover trace headings, paths, grid squares, the opaque terrain, and boundary segments for details. Terrain and boundary permeability consume different grid projections of the same endpoints. <a href="assets/sir-visibility-quint/two-traces.svg#effects-off">effects-off view</a></figcaption></figure>
...
<figure class="combat-quint-diagram" data-diagram-embed="blocker-semantics"><object type="image/svg+xml" data="assets/sir-visibility-quint/blocker-semantics.svg" aria-label="Three grid lanes show an opaque endpoint passing, an opaque interior cell blocking, and an opaque edge blocking" style="width:100%;aspect-ratio:960/450;display:block"><a href="assets/sir-visibility-quint/blocker-semantics.svg"><img src="assets/sir-visibility-quint/blocker-semantics.svg" alt="Three grid lanes show an opaque endpoint passing, an opaque interior cell blocking, and an opaque edge blocking" /></a></object><figcaption>Hover each rule label, blocker, ray, or result for details. Opacity matters only at the precise interior cell and sampled-edge positions named by the two predicates. <a href="assets/sir-visibility-quint/blocker-semantics.svg#effects-off">effects-off view</a></figcaption></figure>
...
<figure class="combat-quint-diagram" data-diagram-embed="footprint-exposure"><object type="image/svg+xml" data="assets/sir-visibility-quint/footprint-exposure.svg" aria-label="Four paths connect two origin cells to two target cells; one blue path survives three red blockers, making the target visible" style="width:100%;aspect-ratio:960/480;display:block"><a href="assets/sir-visibility-quint/footprint-exposure.svg"><img src="assets/sir-visibility-quint/footprint-exposure.svg" alt="Four paths connect two origin cells to two target cells; one blue path survives three red blockers, making the target visible" /></a></object><figcaption>Hover footprint squares, individual pair paths, or the result counter for details. One surviving pair is sufficient: visibility means `visibleSamples &gt; 0`, not that every footprint pair passes. <a href="assets/sir-visibility-quint/footprint-exposure.svg#effects-off">effects-off view</a></figcaption></figure>
...
<figure class="combat-quint-diagram" data-diagram-embed="work-bound"><object type="image/svg+xml" data="assets/sir-visibility-quint/work-bound.svg" aria-label="A four-pair matrix reaches a guard where declared work twelve exceeds maximum eight and returns a truncated result before tracing" style="width:100%;aspect-ratio:960/470;display:block"><a href="assets/sir-visibility-quint/work-bound.svg"><img src="assets/sir-visibility-quint/work-bound.svg" alt="A four-pair matrix reaches a guard where declared work twelve exceeds maximum eight and returns a truncated result before tracing" /></a></object><figcaption>Hover pair entries, admission comparisons, the truncated result, or either work meter for details. The declared sampled-line work controls admission; internal supercover work remains separately visible for review. <a href="assets/sir-visibility-quint/work-bound.svg#effects-off">effects-off view</a></figcaption></figure>
...
<figure class="combat-quint-diagram" data-diagram-embed="evaluator-pipeline"><object type="image/svg+xml" data="assets/sir-visibility-quint/evaluator-pipeline.svg" aria-label="Visibility query flows through footprint pairs, work guard, two line predicates, visible count, and a result record" style="width:100%;aspect-ratio:1120/430;display:block"><a href="assets/sir-visibility-quint/evaluator-pipeline.svg"><img src="assets/sir-visibility-quint/evaluator-pipeline.svg" alt="Visibility query flows through footprint pairs, work guard, two line predicates, visible count, and a result record" /></a></object><figcaption>Hover any pipeline node or transition arrow for details. The functional model introduces no production state; its simulation adapter stores results only to exercise invariants and reachable witnesses. <a href="assets/sir-visibility-quint/evaluator-pipeline.svg#effects-off">effects-off view</a></figcaption></figure>
...
<figure class="combat-quint-diagram" data-diagram-embed="invariant-boundary"><object type="image/svg+xml" data="assets/sir-combat-quint-interactive/invariant-boundary.svg" aria-label="Reachable states remain inside a safety boundary until a mutated transition escapes" style="width:100%;aspect-ratio:720/250;display:block"><a href="assets/sir-combat-quint-interactive/invariant-boundary.svg"><img src="assets/sir-combat-quint-interactive/invariant-boundary.svg" alt="Reachable states remain inside a safety boundary until a mutated transition escapes" /></a></object><figcaption>Hover the safety region, reachable states, altered transition, escaping edge, or failing state for details. The safety boundary encloses checked reachable states; one mutated transition escapes to a concrete violating path. <a href="assets/sir-combat-quint-interactive/invariant-boundary.svg#effects-off">effects-off view</a></figcaption></figure>
...
<figure class="combat-quint-diagram" data-diagram-embed="trace-counterexample"><object type="image/svg+xml" data="assets/sir-combat-quint-interactive/trace-counterexample.svg" aria-label="Separate original-model and altered-model traces compare a twenty-point example with an eighteen-point failing trace" style="width:100%;aspect-ratio:720/280;display:block"><a href="assets/sir-combat-quint-interactive/trace-counterexample.svg"><img src="assets/sir-combat-quint-interactive/trace-counterexample.svg" alt="Separate original-model and altered-model traces compare a twenty-point example with an eighteen-point failing trace" /></a></object><figcaption>Hover either model boundary, initial state, trace edge, result, or scope note for details. The original and altered transition systems are separate: one supplies the representative example, while the other supplies the deliberately failing trace. <a href="assets/sir-combat-quint-interactive/trace-counterexample.svg#effects-off">effects-off view</a></figcaption></figure>
<details id="diagram-transcript-trace-counterexample"><summary>Trace comparison diagram transcript</summary><p>The original model starts at its initial state and reaches harm twenty with vitality eighty, satisfying the representative assertion. The established retention change from 8000 to 7000 creates a separate altered transition system; its own reachable trace reaches harm eighteen with vitality eighty-two and violates that assertion. The red edge is the altered model's failing path, not a nondeterministic branch of the unchanged model and not a claim about every execution.</p></details>
