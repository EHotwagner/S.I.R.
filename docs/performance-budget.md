...
a value.** The representative row's scene cap and this bound are both 5,000 today,
and neither is derived from the other: they are declared apart so that changing one
does not silently move the other, and so that a reader can tell a shared number from
a shared meaning. This bound is workload-independent -- the assertion that enforces
it runs at the product's default scale and nothing in the product tiers overlay cost
by unit count -- so the one declared cell is projected into every row, exactly as the
frame-ceiling column is. A stress-row figure is not published here because none has
ever been measured, and inventing one would be the defect this table exists to remove.
