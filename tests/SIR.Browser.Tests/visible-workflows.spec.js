...
const guardedBudgetTest = test.extend({
  overlayBudgetEvaluated: [async ({}, use) => {
    // BOTH HALVES, FOR BOTH NETS. This guard proves itself ran by moving a counter; the run-level net
    // cannot prove anything from in here, because the case it exists for is a test that never starts.
    // So its INSTALLATION is asserted from the resolved config of this actual run -- the same
    // symmetry, one link along. Checked before the work, so a run with the net unarmed fails whether
    // or not the budget itself would have passed.
    assertTacticalOverlayLayerRunNetArmed(test.info().config);
...
const overlayBudgetTest = (title, body) => {
  // The declaration owns the title too, because the run-level filter refusal has to reconstruct the
  // exact string Playwright greps and cannot read it once the test has been filtered out. Refused at
  // COLLECTION time, so a retitled test fails loudly here instead of silently narrowing that refusal.
  if (title !== tacticalOverlayLayerBudgetTitle)
    throw new Error(`the overlay-layer budget test is declared as ${JSON.stringify(tacticalOverlayLayerBudgetTitle)} in scripts/lib/performance-budget.mjs, and this file declares it as ${JSON.stringify(title)}. The run-level filter refusal rebuilds the title Playwright greps from the declared one, so a title that drifts from it would silently stop being protected.`);
  return guardedBudgetTest(title, { tag: tacticalOverlayLayerBudgetTag }, body);
};
...
import { assertTacticalOverlayLayerBudget, assertTacticalOverlayLayerBudgetWasEvaluatedSince, assertTacticalOverlayLayerRunNetArmed, tacticalFrameBudget, tacticalOverlayLayerBudgetTag, tacticalOverlayLayerBudgetTitle, tacticalOverlayLayerEvaluationMark, tacticalWorkloadBudgetAtScale } from "../../scripts/lib/performance-budget.mjs";
