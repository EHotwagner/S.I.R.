...
junit_pass "$stage/scenario-catalog-native.junit.xml" scenario-catalog-native "$((SECONDS-start))" "$(grep -E 'Scenario catalog (qualification|structural counters|PERF-SMOKE|STRESS-SMOKE)' "$stage/native.log")"
...
stress_line=$(grep -F 'Scenario catalog STRESS-SMOKE:' "$stage/native.log")
counter_value() {
  local name=$1
...
stress_p95_ms=$(sed -E 's/.*p95 ([0-9.]+) ms.*/\1/' <<<"$stress_line")
stress_p99_ms=$(sed -E 's/.*p99 ([0-9.]+) ms.*/\1/' <<<"$stress_line")
stress_duration_samples=$(sed -E 's/.*samples (\[[^]]+\]).*/\1/' <<<"$stress_line")
captured_at=$(date -u +'%Y-%m-%dT%H:%M:%SZ')
stress_definition=scripts/item-184-stress-workload.json
stress_workload_id=$(jq -er '.workloadId' "$stress_definition")
stress_workload_digest="sha256:$(sha256sum "$stress_definition" | cut -d' ' -f1)"

for counter in pathExpansions peakLosSamples peakCombatResolutions sceneNodes; do
  actual=$(counter_value "$counter")
  expected=$(jq -er --arg counter "$counter" '.expectedStructuralCounters[$counter]' "$stress_definition")
  if [[ "$actual" != "$expected" ]]; then
    printf 'stress workload counter mismatch: %s expected %s, observed %s\n' "$counter" "$expected" "$actual" >&2
    exit 1
  fi
done

measurement_scope="production-import-route-simulate-project-scene; mapSize=${map_size}; units=$(counter_value units); pathExpansions=$(counter_value pathExpansions); peakLosSamples=$(counter_value peakLosSamples); peakCombatResolutions=$(counter_value peakCombatResolutions); sceneNodes=$(counter_value sceneNodes); source=scenario-catalog-native.junit.xml; routes=MapEditor.tryImport/MapScale.route/MapScale.tick/Battlefield.scene"
...
jq -n \
  ...
  --argjson stressMeasuredP95Ms "$stress_p95_ms" \
  --argjson stressMeasuredP99Ms "$stress_p99_ms" \
  --argjson stressDurationSamplesMs "$stress_duration_samples" \
  --arg stressWorkloadId "$stress_workload_id" \
  --arg stressWorkloadDigest "$stress_workload_digest" \
  --arg measurementScope "$measurement_scope" \
  ...
  '{
    ...
    claimedBudgetPassed: ($measuredP95Ms <= 20 and $measuredP99Ms <= 50 and $stressMeasuredP95Ms <= 20 and $stressMeasuredP99Ms <= 50 and $pathExpansions <= 4096 and $peakLosSamples <= 256 and $peakCombatResolutions <= 256 and $sceneNodes <= 8000),
    sampleSets: [{
      ...
      currencyToken: "item-184-repair-round-3",
      probeReadbackContaminated: false,
      durationSamplesMs: $durationSamplesMs,
      catchUpFrames: [0]
    }, {
      workloadId: $stressWorkloadId,
      workloadDefinitionDigest: $stressWorkloadDigest,
      workloadClass: "stress-throughput",
      targetFps: 20,
      maxP95Ms: 20,
      maxP99Ms: 50,
      maxCatchUpFrames: 0,
      ...
      measurementMode: "headless",
      capabilities: ["headless-browser"],
      warmupPolicy: "twenty-complete-production-stress-routes",
      samplePolicy: "nearest-rank/20-production-stress-routes",
      capturedAtUtc: $capturedAtUtc,
      currencyToken: "item-184-repair-round-3",
      probeReadbackContaminated: false,
      durationSamplesMs: $stressDurationSamplesMs,
      catchUpFrames: [0]
    }]
