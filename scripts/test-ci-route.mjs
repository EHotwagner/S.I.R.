...
assert.match(jobBody("domain-conformance"), /extract-parts native fable web[\s\S]*run-ci-gate\.sh browser-delivery[\s\S]*git fetch --no-tags origin \+refs\/heads\/main:refs\/remotes\/origin\/main[\s\S]*gates\+=\(rules\)[\s\S]*gates\+=\(spatial\)[\s\S]*gates\+=\(cancellation\)[\s\S]*for gate in[\s\S]*run-ci-gate\.sh "\$gate"[\s\S]*for pid in[^\n]*wait[^\n]*done[\s\S]*gate-domain-conformance/u);
