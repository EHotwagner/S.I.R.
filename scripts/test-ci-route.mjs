...
for (const job of ["route:", "integrity:", "prepare-native:", "prepare-fable:", "prepare-web:", "prepare-server:", "prepare:", "rules:", "spatial:", "cancellation:", "cross-runtime:", "browser:", "documentation:", "evidence:", "pr-verdict:", "full-qualification:"]) assert.match(workflow, new RegExp(`^  ${job}$`, "mu"));
...
assert.match(workflow, /prepare:\n    if: always\(\)[\s\S]*?needs: \[route, prepare-native, prepare-fable, prepare-web, prepare-server\]/u);
for (const part of ["native", "fable", "web", "server"]) assert.match(workflow, new RegExp(`qualify-pr\\.sh prepare-part ${part}`, "u"));
for (const part of ["native", "fable", "web", "server"]) assert.match(workflow, new RegExp(`qualify-pr\\.sh prepare-part ${part}\\n      - if: always\\(\\)`, "u"));
...
assert.match(focusedQualification, /receipt\.outputs\) console\.log\(output\.path\)/u);
assert.ok(focusedQualification.indexOf('wait "$release_pid"') < focusedQualification.indexOf("mapfile -t part_paths"));
assert.ok(focusedQualification.indexOf('tar -xf "$ci_root/parts/$part.tar"') < focusedQualification.indexOf("find src tests -type d -name obj"));
