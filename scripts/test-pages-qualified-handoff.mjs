...
assert.match(pages, /while IFS= read -r path; do[\s\S]*docs\/evidence\/production-build-receipt-v1\/\*\.json[\s\S]*tar -xf "\$archive" --no-same-owner/u);
assert.match(ci, /handoff=artifacts\/qualified-site-handoff/u);
assert.match(ci, /tar -cf "\$handoff\/protected-qualified-site\.tar" --[\s\S]*artifacts\/site[\s\S]*"\$source_receipt"[\s\S]*"\$build_receipt"[\s\S]*"\$conformance_receipt"/u);
