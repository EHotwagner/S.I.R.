...
const result = spawnSync(
  ...
  { cwd: process.cwd(), env: { ...process.env, SIR_DIAGNOSTIC_SELF_TEST: "1" }, encoding: "utf8" },
);
const output = `${result.stdout}\n${result.stderr}`;
if (result.status === 0 || !output.includes("unexpected console, page, or network diagnostics")) {
