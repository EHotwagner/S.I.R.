...
- PD-006 [AC-001] [AC-002] [FR-006] [DEC-001] complete: Register shell/bootstrap, tactical-environment/eager, rules-explorer/deferred, and docs/deferred; source validation rejects feature module imports not owned by the registry/import edge.
- PD-007 [AC-004] [FR-007] [DEC-005] complete: Parse Fable/Vite import edges and emitted Vite manifest membership, then fail when a deferred module is statically reachable from the entry or absent from its declared logical chunk.
...
- PD-009 [AC-005] [FR-009] complete: Provide protected environment mutations `eager-import`, `missing-chunk`, and `stale-identity`; each changes the actual gate subject and the mutation harness requires a non-zero result containing that subject.
