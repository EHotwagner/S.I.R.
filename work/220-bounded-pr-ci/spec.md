...
- AC-002 [US-001] [US-002] [FR-003] [FR-004]: Given any PR path set, when required qualification runs on `ubuntu-latest`, then the deterministic join reports an actionable required verdict within five minutes and preserves gate-specific attribution.
...
- FR-003: Required PR feedback MUST complete within 300 seconds from runner start to actionable deterministic join verdict on the declared `ubuntu-latest` runner for the representative route matrix. (Stories: US-001, US-002; Acceptance: AC-002)
