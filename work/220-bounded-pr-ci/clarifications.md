...
- CQ-005 → a route receipt records runner start and deterministic per-phase durations; the join compares required-path elapsed time with 300,000 ms. Queue time is recorded separately when GitHub exposes it, and product performance remains outside this budget.
...
- **DEC-005** [AMB:AMB-005] [FR-003] [FR-011]: Budget the latest candidate from focused runner start to join verdict at 300,000 ms, record phase/gate/critical-path/runner-minute facts, and never mix this runner contract with gameplay performance.
