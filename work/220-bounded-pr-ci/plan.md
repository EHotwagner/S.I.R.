...
- PD-003 [AC-002] [FR-003] [DEC-005] in-progress: Keep the 300,000 ms enforced PR boundary and require the final representative cross-cutting acceptance run to complete within 240,000 ms. Consolidate ordinary native preparation into one Release solution graph whose source-bound outputs downstream conformance gates consume without configuration-duplicate builds. Launch each fixed source-mutation cohort as a required source-bound helper directly after routing, in parallel with native/Fable/web/server/docs preparation: the nine-subject spatial helper retains its existing proof, while `cancellation-mutations` owns the fixed cancellation subject mutation. Because the real worker probe requires the current authoritative replay fixture before any prepared-native artifact exists, the early helper performs one explicit minimal `SIR.Domain.Tests` Release build under the named `cancellation-fixture` exception, then builds the mutated client and requires the runtime cancellation diagnostic; it must fail if that fixture generator is omitted or if the mutation is not observed. Ordinary `spatial` and `cancellation` retain their restored exact-runtime conformance/smoke subjects after verified producer inputs arrive; the deterministic join requires the matching helper receipt whenever either gate is selected. Browser capacity retains the dispatched pair-accounting contract `max(1, floor(availableParallelism / 2))` inside every runner, because every shard owns an independent server and browser worker as well as its own port and JUnit fragment. The fixed Slow-3G/4x-CPU production-delivery proof remains a distinct source-bound helper over verified web/server artifacts. The growing general inventory is deterministically split at test granularity across four independently isolated hosted jobs, one server/browser pair per runner; four is the current workflow concurrency allocation rather than an inventory-size ceiling, so all discovered tests remain complete, disjoint, and divided as the inventory grows. The four fragments are strictly merged, and every helper must be an explicit `pr-verdict.needs` dependency; focused inversions fail if any dependency edge, mutation proof, or fragment is removed. The explicit LongPolling disconnect/reconnect journey starts the completed connection lifecycle directly and retains authoritative resync without a second stop or stale connection-id 404. Every fragment must be well-formed, non-empty, internally count-consistent JUnit before deterministic merge; unreadable, malformed, empty, or count-drifted fragments fail closed. Local/full runs preserve the complete one-shard default and an explicit capacity-bounded override. Four-runner browser fan-out, early cancellation-mutation timing, isolated fixture-generator ownership, and their focused inversions are complete; final exact-head hosted acceptance remains outstanding.
...
- The round-one hosted head again passed every substantive gate but joined at 279,159 ms. Browser improved to 108,355 ms total / 84,714 ms test and spatial improved to 125,878 ms total / 84,383 ms build / 13,734 ms test, but the workflow still serialized the roughly 108-second preparation/integrity stage before the fixed nine-mutant spatial build cohort. The bounded round-two design makes that mutation cohort a required, source-bound helper gate launched directly after routing, parallel to preparation, while keeping final native/Fable spatial conformance in the ordinary spatial gate. The join must reject a missing, stale, malformed, failed, duplicate, or unexpected helper receipt and hosted acceptance remains pending.
- Same-critic round-one review also rejected `availableParallelism - 1` because three browser-plus-server pairs oversubscribe four reported lanes. Round two restores `floor(availableParallelism / 2)` exactly. To avoid returning the throttled fixed proof to the growing general inventory's serial path, production delivery becomes a separate required browser helper over the same verified web/server artifacts; both jobs use the safe pair capacity, run concurrently without depending on integrity, and remain independently attributable at the typed join.
- The round-two hosted topology made every non-general-browser subject green and reduced the aggregate to 269,386 ms, but remained 29,386 ms above the acceptance target. The general browser job was still the serial continuation of the 113,993 ms web producer and consumed 110,987 ms on one four-lane runner while scheduling two complete server/browser pairs. Its 20/20 test-granular split was balanced, yet one explicit LongPolling disconnect/reconnect journey emitted a stale `/hub/game?id=...` 404 after the client stopped the same connection once in `disconnect` and again in `reconnect`; five focused reruns passed, so the hosted failure remains authoritative and must be repaired rather than whitelisted.
- Round-three preserves pair accounting inside every runner and moves the two balanced general halves onto two isolated hosted runners, each running exactly one server/browser pair and one globally indexed fragment. This is a bounded workflow-concurrency allocation, not a project-size ceiling: the inventory is still discovered and split at test granularity, no test is omitted as it grows, and the fragment merge must prove complete disjoint coverage. The ordinary `browser` receipt and a required general-browser helper receipt remain independently attributable. `pr-verdict.needs` must name `spatial-mutations`, `browser-delivery`, and the new general-browser helper explicitly; deleting any one dependency must make the focused route contract red before hosted execution.
- The round-three exact head passed all 16 typed substantive receipts and closed the hosted reconnect 404, but the verdict remained red at 276,119 ms, 36,119 ms above the 240,000 ms acceptance target. The two globally indexed browser halves were balanced by count yet unequal by hosted duration: the ordinary browser path consumed 123,741 ms total / 93,271 ms test while the helper consumed 79,912 ms total / 57,400 ms test. Cancellation remained 118,384 ms total, including a 56,417 ms fixed mutation build after its producer stage. These are measured critical-path subjects, not permission to weaken the target or retry an unchanged head.
- Round four therefore splits the complete growing general inventory across four isolated hosted runners with global indices 1/4 through 4/4, retaining one server/browser pair per runner and strict complete/disjoint cross-runner merge. It also moves only the fixed cancellation mutation proof to a new required helper launched directly after routing; the ordinary cancellation gate continues the restored real-worker smoke over prepared artifacts. Route-contract inversions must reject removal or failure of either new browser dependency, the cancellation helper dependency, or the mutation-only proof. This preserves all subjects while pulling the measured browser and cancellation work off the serial critical path.
- The round-four hosted head validated the timing design: all browser, delivery, ordinary cancellation, spatial, and other substantive receipts passed, and the aggregate completed in 231,069 ms with 68,931 ms outer headroom. The sole failure was functional and fail-closed: the early cancellation helper reached its real worker probe without the `SIR.Domain.Tests` executable used to emit the current replay fixture, so `--no-build` failed before cancellation semantics ran. The helper already completed setup plus mutated client build in 97,407 ms and starts immediately after routing. Round five keeps that concurrency and adds only the explicit minimal fixture-generator build to the helper's traced exception inventory; the ordinary prepared smoke remains unchanged and no timing target or gate is weakened.
