...
- PD-003 [AC-002] [FR-003] [DEC-005] in-progress: Keep the 300,000 ms enforced PR boundary, require the final representative cross-cutting acceptance run to complete within 240,000 ms, consolidate native preparation into one Release solution graph whose source-bound outputs all downstream gates consume without configuration-duplicate builds, execute the fixed spatial mutation matrix with bounded concurrency over isolated source/output roots while retaining every named mutant and diagnostic, and balance the production browser inventory at test granularity across isolated single-worker Playwright shards. Each shard owns an independent server and browser, so automatic capacity is `max(1, floor(availableParallelism / 2))`, not the raw CPU count; this reserves one reported CPU slot for each of the shard's two independently scheduled processes and prevents server starvation. Independent ports and one deterministically merged JUnit receipt preserve isolation and attribution. Local runs preserve a one-shard default and an explicit isolation-capacity-bounded override. Focused implementation and distribution/inversion evidence are complete; exact-head hosted acceptance remains outstanding.
