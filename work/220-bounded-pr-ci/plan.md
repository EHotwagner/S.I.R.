...
- PD-003 [AC-002] [FR-003] [DEC-005] in-progress: Keep the 300,000 ms enforced PR boundary and require the final representative cross-cutting acceptance run to complete within 240,000 ms. Consolidate native preparation into one Release solution graph whose source-bound outputs downstream conformance gates consume without configuration-duplicate builds. Launch the fixed nine-subject spatial mutation proof as its own source-bound helper gate directly after routing, in parallel with native/Fable/web/server/docs preparation; ordinary `spatial` retains exact native/Fable conformance after its verified producer inputs arrive, and the deterministic join requires both receipts whenever spatial is selected. This removes an independent fixed proof from the second serial DAG stage without weakening or hiding it. Browser capacity uses the dispatched pair-accounting contract `max(1, floor(availableParallelism / 2))`, because every shard owns an independent server and browser worker as well as its own port and JUnit fragment. The fixed Slow-3G/4x-CPU production-delivery proof is a distinct source-bound helper gate over the same verified web/server artifacts, concurrent with the growing general browser inventory. The growing general inventory is deterministically split at test granularity across two independently isolated hosted jobs, one server/browser pair per runner, so the current workflow allocation does not impose a test-count ceiling or oversubscribe either runner; the two fragments are joined strictly and future inventory remains divided rather than capped. The deterministic verdict depends explicitly on every helper job that supplies a required receipt, and focused inversions must fail if either dependency edge is removed. The explicit LongPolling disconnect/reconnect journey must not stop an already stopped connection a second time: reconnect starts the completed connection lifecycle directly and retains the authoritative resync proof, preventing the stale connection-id request observed only under hosted timing without allowing its 404 diagnostic. Every fragment must be well-formed, non-empty, internally count-consistent JUnit before deterministic merge; unreadable, malformed, empty, or count-drifted fragments fail closed. Local/full runs preserve the complete one-shard default and an explicit capacity-bounded override. Round-three general-browser fan-out, dependency inversions, deterministic reconnect repair, and exact-head hosted acceptance remain outstanding.
