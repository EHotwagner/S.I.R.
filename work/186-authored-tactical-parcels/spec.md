...
- AC-008 [US-004] [FR-012]: Given representative and maximum assembly, validation, preview, invalidation, and combat-query workloads, when Release qualification and protected-subject mutations run, then structural/timing budgets pass and ignoring edge state, content identity, dependency locality, or destruction bounds makes the owning gate red.
...
- FR-012: Release qualification MUST traverse production assembly, validation, editor preview, local invalidation, and representative combat-query routes with declared maximum scales, structural counters, host facts, and latency budgets; each added or modified gate MUST retain a mutation proving it fails when edge state, content hash, bounded destruction cost, or local dependency invalidation is ignored. (Stories: US-004; Acceptance: AC-008)
