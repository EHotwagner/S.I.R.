...
Initial-boot budget v2 (2026-08-15) caps both the static application entry and the measured default production browser route at 1,250,000 bytes. The browser observation uses CDP Slow 3G and 4x CPU throttling, asserts that RulesExplorer is absent before activation, then loads that deferred chunk through the normal UI. This is a versioned initial-route contract, not a forever-global product limit: future growth must cross a real deferred boundary or explicitly version and rebaseline the ceiling. A tightened static ceiling and an oversized browser-response mutation must each make their owning gate red.
