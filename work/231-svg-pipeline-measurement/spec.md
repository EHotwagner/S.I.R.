...
- SB-004: Artifacts bind the exact candidate, fixture definition digest, journey identity, browser/runtime/capability facts, deterministic summary, and clean-archive-durable content-addressed raw trace bytes.
...
- AC-002 [US-003] [FR-004] [FR-005]: Given two fixtures with the same small visible viewport, density, and unit count but materially different global extent/list scale, plus a separate controlled global-unit pair, when their idle and interaction journeys run, then the summary preserves global and visible counters and compares their work without treating the larger fixture as unsupported.
...
- FR-002: Each journey artifact MUST separately report worker compute and transfer, scene projection and allocation, Elmish update and React reconciliation, SVG style/layout/paint/compositor activity, DOM counts by layer, long tasks, dropped frames, and input-to-paint latency; frame and latency evidence MUST come from Chromium `AnimationFrame`, user `EventDispatch`, paint, and presentation trace events without injecting a sampling callback into the measured renderer, and unavailable capabilities MUST remain explicit rather than merged or invented. (Stories: US-001, US-002; Acceptance: AC-001)
- FR-003: Every artifact MUST bind the exact candidate commit/tree and build identity, fixture schema/id/digest, journey id, Chromium and runtime versions, host/capability facts, content-addressed retained trace path/digest, start/end timestamps, and result; a clean archive MUST contain every declared compressed trace and validation MUST fail on absent, malformed, or changed decompressed bytes. (Stories: US-002; Acceptance: AC-001)
- FR-004: The versioned fixture matrix MUST independently vary map extent, visible density, global unit count, route/overlay complexity, event rate, and supporting-list size through a declared one-factor controlled pair for every axis, with declared expected scales and production-observed structural counters; visible density MUST equal viewport-intersecting production unit glyphs, its pair MUST change that observed count, the global-unit pair MUST hold that observed count constant, and summary validation MUST fail closed on either escape. (Stories: US-002, US-003; Acceptance: AC-002)
...
- FR-009: The full production Chromium matrix MUST be independently runnable and focused, support selecting fixtures/journeys and an artifact directory, and MUST not run in the small-PR CI route; SVG-harness-only changes MAY use a fixed-function documentation/evidence route that revalidates retained evidence, MUST keep broad CI policy/runner files conservatively cross-cutting, and MUST retain the 240-second target with 60 seconds of headroom. (Stories: US-002; Acceptance: AC-005)
