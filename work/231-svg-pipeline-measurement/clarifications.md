...
- CQ-002 → Combine explicit in-page User Timing marks around real user actions, `PerformanceObserver` long-task/event observations, DOM/layer snapshots, heap observations, and a Chromium DevTools trace containing timeline, Blink user timing, V8, loading, rendering, and screenshots. Every metric carries `available`, `value`, and a bounded reason when unavailable.
...
- **DEC-002** [CQ-002] [AMB:AMB-002] [FR-002] [FR-003]: A schema-v1 summary plus raw Chromium trace is authoritative; unavailable browser capabilities remain explicit unavailable observations and never become zero-valued successes.
