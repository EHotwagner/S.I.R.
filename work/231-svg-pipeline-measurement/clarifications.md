...
## Answers
- CQ-001 → Generate canonical `SIR-MAP 2` texts from versioned fixture definitions, import them through the visible production map picker, and drive the real workspace controls. Fixture metadata declares the independent axes; the imported document reports global counters, while production glyph bounds intersected with the tactical SVG viewport report the visible working set after camera setup. Authority is refused unless the visible-density pair changes that observed count and the global-unit pair holds it constant.
...
## Decisions
- **DEC-001** [CQ-001] [AMB:AMB-001] [FR-001] [FR-004] [FR-005]: Versioned JSON fixture definitions generate maps that enter through the production import control; journeys and camera setup use only production pointer, keyboard, menu, timeline, and workspace controls, and viewport-intersecting production glyph bounds provide the visible-density observation validated across controlled pairs.
