...
- PD-009 [AC-005] [FR-009] complete: `node scripts/measure-svg-pipeline.mjs --fixtures ... --journeys ... --out ...` remains opt-in. Changes confined to the five SVG-specific harness files plus documentation take the fixed-function documentation/evidence route; its hosted owner runs the focused retained-evidence validator but never regenerates the expensive Chromium matrix. A routed `work/<id>/hosted-verification.sh` subject is required to be a regular, readable, executable file, its exit status propagates, and exact missing/non-executable mutations fail closed. Broad route, runner, test-policy, and workflow files remain cross-cutting. Both performance and cross-cutting routes enforce the 240-second target and 60-second headroom, and workflow assertions/mutants require the performance route's web/docs producers. The web producer transports the exact lockfile-installed Playwright runtime as receipt- and manifest-bound outputs, so browser consumers reuse those bytes instead of repeating a full dependency install on the critical path; removal of any runtime output or restoration of a consumer install fails the route contract. Because every route selecting `spatial` also selects `rules`, those two receipts execute sequentially in the same consumer job and share checkout/setup; this removes an independently queued runner without dropping either typed receipt, and deleting the co-scheduled spatial invocation fails the route contract. Receipt binding is scoped to each subject's declared `preflight_parts`, so the Fable manifest present for the later spatial command cannot contaminate the native-only rules receipt; an executable extra-manifest fixture proves isolation and missing requested subjects fail closed. The cross-runtime stress qualification constructs its immutable 80x80/200-unit package once and reuses it across warm-up and measured iterations, removing fixture-construction allocation pressure while preserving the 100/125 ms p95/p99 ceilings and the required browser-WASM build.
