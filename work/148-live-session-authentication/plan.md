...
## Performance Intent
No producer-owned typed timing target exists for this work item. Apply the performance-first gate without inventing one: before implementation, capture a focused production browser connect/advance/reconnect baseline, preserve one token validation and one session lookup per connection/intent as structural counters, and label the local headless result as non-compositor evidence. Any required live-compositor release evidence remains a protected-host obligation.
