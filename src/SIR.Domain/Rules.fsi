...
type RuleMetadata =
    { Id: RuleId
      Title: string
      Status: RuleStatus
      SemanticKind: RuleKind
      Statement: ControlledStatement
      Rationale: string
      Dependencies: RuleId list
      Supersedes: RuleId list
      RuleSource: SourceRef option
...
type FormulaExpr =
    | Constant of TypedValue
    | Input of name: string * kind: RuleValueKind * unitName: string
    | Add of FormulaExpr * FormulaExpr
    | Subtract of FormulaExpr * FormulaExpr
    | Multiply of FormulaExpr * FormulaExpr
    | Divide of FormulaExpr * FormulaExpr
    | MinimumOf of FormulaExpr * FormulaExpr
    | MaximumOf of FormulaExpr * FormulaExpr
    | Clamp of minimum: FormulaExpr * maximum: FormulaExpr * value: FormulaExpr
    | LessThanOrEqual of FormulaExpr * FormulaExpr
    | IfThenElse of condition: FormulaExpr * whenTrue: FormulaExpr * whenFalse: FormulaExpr

type TransitionContract =
    { Phase: string
      Preconditions: RuleId list
      Reads: string list
      Effects: string list
      Events: string list }

type AlgorithmContract =
...
type RuleSemantics =
    | FactSemantics of TypedValue
    | PredicateSemantics of FormulaExpr
    | FormulaSemantics of resultKind: RuleValueKind * resultUnit: string * FormulaExpr
    | TransitionSemantics of TransitionContract
    | AlgorithmSemantics of AlgorithmContract
    | NarrativeSemantics
...
type RuleApplication =
    { ApplicationId: string
      RuleId: RuleId
      Operands: (string * TypedValue) list
      Outcome: TypedValue
      Children: RuleApplication list
      EventId: string
...
type RulePackageIdentity =
    { SchemaVersion: int32
      ...
      SourceCommit: string
      ImplementationDigest: byte array
      SemanticDigest: byte array
      ManifestDigest: byte array }
