...
module ExperienceSamples =
    ...
    let private tacticalDensitySample unitCount =
        ...
        { Id = "tactical-density-" + string unitCount
          ...
          Family = FastStartTeaching
          Lesson = "Inspect production scene behavior under a dense deterministic workload."
          Highlights =
          ...
          DesignNotes = [ "Qualification-only fixture; excluded from the seven-scenario teaching catalog." ]
          MapText = String.concat "\n" lines + "\n" }

    let tryMap id =
        (maps @ [ stressMap; tacticalDensitySample 100; tacticalDensitySample 200 ])
        |> List.tryFind (fun sample -> sample.Id = id)
